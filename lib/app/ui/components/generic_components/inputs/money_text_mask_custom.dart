...
class MyMaskedTextController extends TextEditingController {
  MyMaskedTextController(
      {String? text, this.mask, Map<String, RegExp>? translator})
      : super(text: text) {
    this.translator =
        translator ?? MyMaskedTextController.getDefaultTranslator();
  ...
  String _applyMask(String mask, String value) {
    ...
    while (true) {
      ...
      if (this.translator?.containsKey(maskChar) ?? false) {
        if (this.translator?[maskChar]?.hasMatch(valueChar) ?? false) {
          result += valueChar;
          maskCharIndex += 1;
        }
