...
import 'package:cGamer/app/utils/api_utils/discord_utils/models/discord_connection.dart';
import 'package:cGamer/app/utils/api_utils/discord_utils/models/discord_partial_guild.dart';
import 'package:cGamer/app/utils/api_utils/discord_utils/utils/discord_utils.dart';
import 'package:cGamer/app/utils/api_utils/discord_utils/models/discord_user.dart';

import 'package:cGamer/app/utils/api_utils/discord_utils/utils/exceptions.dart';
...
class DiscordManager {
  ...
  Future<bool> getAuthorizationGrant(BuildContext context) async {
    if (scopes == null)
    ...
    if (await canLaunch(url)) {
      await launch(url, forceSafariVC: false, forceWebView: Platform.isIOS);
    } else {
    ...
    await uriLinkStream.firstWhere((uri) {
  ...
  Future<bool> _getToken(bool isRefresh) async {
    ...
    try {
      response = await _client
          .post(Uri.https('discord.com', '/api/oauth2/token'), body: body);
    } catch (e) {
      print(e.toString());
      return false;
    }
  ...
  Future<bool> revokeToken(DiscordTokenType type) async {
    ...
    try {
      response = await _client.post(
          Uri.https('discord.com', '/api/oauth2/token/revoke'),
          headers: {
            'Authorization':
  ...
  Future<DiscordUser?> getUsersMe() async {
    Response response =
        await get(Uri.https('discord.com', '/api/v6/users/@me'));
    discordUser = DiscordUser.fromRawJson(response.body);
    return discordUser;
  }

  Future<dynamic> addMemberToDiscord(String userId) async {
    ///roles/917271933412839454
    Response response = await put(
      Uri.https(
          'discord.com', '/api/v6/guilds/842946978006695936/members/$userId'),
    );
    return response;
  }

  Future<dynamic> setRolePrime(String userId) async {
    Response response = await put(
      Uri.https('discord.com',
          '/api/v6/guilds/842946978006695936/members/$userId/roles/917271933412839454'),
    );
    return response;
  }
  ...
  Future<List<DiscordPartialGuild>> getUsersMeGuilds(
    ...
    Response response =
        await get(Uri.https('discord.com', '/api/v6/users/@me/guilds'));
  ...
  Future<List<DiscordConnection>> getUsersMeConnections() async {
    Response response =
        await get(Uri.https('discord.com', '/users/@me/connections'));
  ...
  Future<Response> get(Uri apiEndpoint) async {
    if (credentials == null)
      throw DiscordCredentialsNotSetException("Invalid credentials in get");
    if (credentials?.expireInstant?.isAfter(DateTime.now()) ?? false)
      await refreshToken();
    try {
    ...
    return Response('{"message": "Algo de errado aconteceu"}', 500);
  }

  Future<Response> put(Uri apiEndpoint) async {
    if (credentials == null)
      throw DiscordCredentialsNotSetException("Invalid credentials in get");
    if (credentials?.expireInstant?.isAfter(DateTime.now()) ?? false)
      await refreshToken();
    try {
      var body = jsonEncode({'access_token': '${credentials?.accessToken}'});
      //'Authorization': '${credentials?.tokenType} ${credentials?.accessToken}'
      return _client.put(apiEndpoint, body: body, headers: {
        'Authorization':
            'Bot OTE2NTIzOTMwODY3NjAxNDg5.YarZbQ.xc8EtHajj45oOc64ReG7-S9-gQ8',
  ...
  String buildAvatarUrl({DiscordImageFormat format = DiscordImageFormat.PNG}) {
    ...
    if (discordUser?.avatar == null) {
      ...
      final int discriminator =
          int.parse(discordUser?.discriminator ?? "0") % 5;
      return cndEndpoint +
          "embed/avatars/" +
