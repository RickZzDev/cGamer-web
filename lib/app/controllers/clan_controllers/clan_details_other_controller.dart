...
class ClanDetailsOtherController extends GetxController {
  final ClanDetailsRepository repository;

  ClanDetailsOtherController({required this.repository});
  ...
  bool userHasClan = Get.arguments['userHasClan'];
  // earn trophy area
  ...
  int? getTimeLeft(String? date) {
    if (date == null) {
      ...
      DateTime dateLastAction = new DateFormat("dd/MM/yyyy hh:mm").parse(date);
      DateTime dateNextAction = dateLastAction.add(Duration(minutes: 10));
      return dateNextAction.millisecondsSinceEpoch;
    }
  }
